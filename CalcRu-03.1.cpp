...
void fpiCalc(char symb); //вычисление с числом pi, pi+pi, pi-pi 
...
void feCalc(char symb); //вычисление с числом e, e+e, e-e
...
{	
	...
	{
		cout<<"\nКалькулятор> Введите символ для выбора справки по модулю 2 из 6\nКалькулятор> ";
			cin>>sym;
		fhelper(sym);
	}
	...
	if(conh == 11)
	{
		cout<<"\nКалькулятор> Этот модуль для вычислении чисел PI\nПрототип (PI (+,-,*,/) PI)";
		cout<<"\n\nКалькулятор> ";
			cin>>sym;
		fpiCalc(sym);
	}
	...
	if(conh == 10) //Вычисление с моудлями которые возводятся в степень заданую пользователям 
	{
		cout<<"\nКалькулятор> Введите первое число\nКалькулятор> ";
		cin>>numd1;
	...
	if(conh == 11) //вычисление с числом e
	{
	...
	if(conh == 13)
	{
		...
		cout<<"\nКалькулятор> Этот моудль для вычисление с числами e\nПрототип(e (+,-,*,/) e)\n";
		cout<<"Калькулятор> ";
			cin>>sym;
		feCalc(sym);
	}	
...
void fhelper(char synb) //справочник по обычному калькулятору
{
	...
	case '5':
		...
		break;
	case '6':
		cout<<"\nКалькулятор>\tЭтот модуль для вычисление чисел pi\n\tНапример pi + pi или pi / pi\n";
		break;	
	}
}
...
void fhelperPRO(char synb)//справочник по расширеннному калькулятору
{
	switch(synb)
	{
		...
		case '6':
		cout<<"\nКалькулятор>\tЭтот модуль для вычисление чисел e\n\tНапример e + e или e / e\n";
		break;	
	}
}
...
void fpiCalc(char symb) //вычисление с двуям числами pi;
{
	double PI = 3.141592653589793;
	double result;
...
void fmoduleCalcDeg(double fnum, double snum, char symb)
{
	...
	switch(symb)
	{
		...
		case '-':
		if(res4 > res3){result = res4 - res3; cout<<"Калькулятор> "<<result<<endl;}
...
void fe1(double number) //вычисление вашего числа с числом е(+,-,*,/);
{
...
void feCalcPlus(double fnum, double snum, char symb) //положительное число e;
{
	double e = 2.7182818284;
	double result;
...
void feCalcMinus(double fnum, double snum, char symb) //отрицательное число e;
{
	double e = 2.7182818284;
	double result;
...
void feCalcM(double fnum, double snum, char symb) //умноженое число e;
{
	double e = 2.7182818284;
	double result;
...
void feCalcDiv(double fnum, double snum, char symb) //деленое число e;
{
	double e = 2.7182818284;
	double result;
...
void feCalc(char symb) //вычисление с числом e;
{
	double e = 2.7182818284;
	double result;
